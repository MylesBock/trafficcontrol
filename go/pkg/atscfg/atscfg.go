...
// Server is a tc.Server for the latest pkg/tc and traffic_ops/vx-client type.
...
// DeliveryService is a tc.DeliveryService for the latest pkg/tc and traffic_ops/vx-client type.
// This allows atscfg to not have to change the type everywhere it's used, every time ATC changes the base type,
// but to only have to change it here, and the places where breaking symbol changes were made.
type DeliveryService tc.DeliveryServiceNullableV30

// ToDeliveryServices converts a slice of the latest pkg/tc and traffic_ops/vx-client type to the local alias.
...
// ToServers converts a slice of the latest pkg/tc and traffic_ops/vx-client type to the local alias.
...
// This is provided as a convenience and unified structure for users. The pkg/atscfg library doesn't actually use or return this. See ATSConfigFileData.
type CfgFile struct {
	Name string
	Path string
	Cfg
