...
if [ $# -gt 0 ]; then
	...
	# TODO: move these build scripts over to scripts/{project}/build or build/{project}/build
...
for p in ${projects}; do
	...
	p="${p%/}"
	pwd
	find ../.. -wholename "*${p}/build/build_rpm.sh"
  bldscript=$(find ../.. -wholename "*${p}/build/build_rpm.sh")
	if [ -z "$bldscript" ]; then
