...
var ServerName = "traffic_ops" + "/" + about.About.Version
...
// GetWrapAccessLog returns a Middleware which writes to the Access Log (which is the pkg/log EventLog) after the HandlerFunc finishes.
...
// WrapAccessLog takes the cookie secret and a http.Handler, and returns a HandlerFunc which writes to the Access Log (which is the pkg/log EventLog) after the HandlerFunc finishes.
