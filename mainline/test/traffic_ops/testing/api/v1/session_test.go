...
import (
	"github.com/apache/trafficcontrol/pkg/client"
	"time"
...
func SetupSession(toReqTimeout time.Duration, toURL string, toUser string, toPass string) error {
	...
	NoAuthTOSession = client.NewNoAuthSession(toURL, true, "to-api-v1-client-tests", true, toReqTimeout)
	TOSession, _, err = client.LoginWithAgent(toURL, toUser, toPass, true, "to-api-v1-client-tests", true, toReqTimeout)
	return err
}
...
func TeardownSession(toReqTimeout time.Duration, toURL string, toUser string, toPass string) error {
	var err error
	toReqTimeout = time.Second * time.Duration(Config.Default.Session.TimeoutInSecs)
	TOSession, _, err = client.LogoutWithAgent(toURL, toUser, toPass, true, "to-api-v1-client-tests", true, toReqTimeout)
